...
//    $tengriurl = $_POST['url'];
    ...
    foreach ($a as $b)
    {
        $dom20 = new Dom;
        {
            ...
            {
                if( substr($content->href,0,5) != 'https')
                {

                echo "<pre>",var_dump( substr($content->href,0,5)),"</pre>";
                $dom = new Dom;
                $dom->loadFromUrl('https://kaz.tengrinews.kz'.$content->href);
                $a = $dom->find('.tn-link');
//                foreach ($a as $b)
//                {
//                    $dom2 = new Dom;
